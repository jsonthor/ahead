...
export async function fetchRouteAttempts(
  ...
  client: any,
  asOf: string,
  lookbackDays = DIRECTION_HISTORY_DAYS,
): Promise<RouteResponseAttempt[]> {
  const from = shiftDate(asOf, -lookbackDays);
  const to = shiftDate(asOf, 1);
...
export async function fetchDirectionResponse(
  ...
  const attempts = await fetchRouteAttempts(client, asOf, DIRECTION_WINDOW_DAYS);
  return observeRouteResponse(attempts, asOf);
}
